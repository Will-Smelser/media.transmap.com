...
<script>
    ...
    var store = {
        ...
        setItem : function(key,obj){
            //store locally
            return this.storage.set(key,this.jsonToStr(obj));
        },
        /* update the attribute of an item in localStorage */
        setAttribute : function(key, attr, value){
            var obj = JSON.parse(this.storage.get(key));
            obj[attr] = value;
        ...
        //initialize
        init : function(map, marker, fusionTableId, store){
            var scope = this;
    ...
    function initialize() {
        ...
        var store = $.initNamespaceStorage('beta4');
        store.init(map, marker, fusionTableId store);
